...
class Color {
  ...
  harmony() {
    ...
    return {
      ...
      triadic: harmonize(120, 240, 120),
      tetradic: harmonize(90, 270, 90),
  ...
  shades(num: number = 6) {
    ...
    const increment = [180, 170, 160, 150, 140, 130][num - 3];
    const step = increment / quantity;
...
class Filter {
  ...
  solve() {
    ...
    const { code, filter } = this.css(result.values);
    return {
  ...
  css(filters: any) {
    const fmt = (idx: any, multiplier = 1) => (Math.round(filters[idx] * multiplier));
    const filter = `invert(${fmt(0)}%) sepia(${fmt(1)}%) saturate(${fmt(2)}%) hue-rotate(${fmt(3, 3.6)}deg) brightness(${fmt(4)}%) contrast(${fmt(5)}%)`;
    const code = `filter: ${filter};`;
    return { code, filter };
  }
}
...
export function getColorShades(hex: string | null, num: number = 6): ColorData {
  if (hex) {
    ...
    return { shades: color.shades(num) };
  } else return { shades: [] };
};
...
export const getColorFilter = (hex: string | null): ColorData => {
  ...
  if (hex) {
    ...
    if (rgb) {
      ...
      if (result) {
        filter = result;
        message = `Loss: ${Math.round((result.loss + Number.EPSILON) * 100) / 100}% `;
      }
      if (result.loss < 1) message += 'Perfect';
      else if (result.loss < 5) message += 'Close';
...
export const getColorTheme = (hex: string | null, num: number): ColorData => {
  if (hex) {
    ...
    if (rgb) {
      ...
      const { complementary, analogous, tetradic } = harmony;
      return {
        theme: {
          primary: hex,
          ...shadeData,
